<form action="{{route('dashboard.product_edit')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        <label for="exampleFormControlInput1" class="form-label">Le produit sera t-il visible sur la
            boutique</label>
        <select name="isActivated" id="">
            <option value=1 @selected($product->isActivated == 1) >oui</option>
            <option value=0  @selected($product->isActivated != 1) >non</option>
        </select>
    </div>
    <div class="mb-3">
        <label for="exampleFormControlInput1" class="form-label">Le produit sera t-il en solde</label>
        <select name="isSale" id="">
            <option value=1 @selected($product->isSale == 1)>oui</option>
            <option value=0 @selected($product->isSale != 1)>non</option>
        </select>
    </div>
    <div class="mb-3">
